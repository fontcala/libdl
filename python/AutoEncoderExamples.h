...
class AutoEncoderSkipExample
{
    // Doesnt make sense to add skip connections in an autoencoder (unless purpose is denoising or sth like that)
    const size_t mInputHeight;
    const size_t mInputWidth;
    ConvLayer<ReLUActivation> conv1;
    ConvLayer<ReLUActivation> conv2;
...
public:
    AutoEncoderSkipExample(const size_t aInputHeight,
                           const size_t aInputWidth,
                           const size_t aInputDepth,
                           const size_t aFilterSize1,
                           const size_t aFilterSize2,
    ...
    void Train(const MatrixXd &aInput, const MatrixXd &aLabels, const double aLearningRate, const size_t aEpochNum)
    {
        ...
        for (size_t vEpoch = 0; vEpoch < aEpochNum; vEpoch++)
        {
            ...
            for (const auto &vIndex : vIndexTrainVector)
            {
                ...
                tran8.ForwardPass();
                tran7.ForwardPass();
                tran7.ForwardAdditionSkipConnection(conv7);
                tran6.ForwardPass();
                tran5.ForwardPass();
                tran4.ForwardPass();
                tran4.ForwardAdditionSkipConnection(conv4);
                tran3.ForwardPass();
                tran2.ForwardPass();
                tran1.ForwardPass();
                loss.ForwardPass();
                ...
                conv8.BackwardPass();
                conv7.BackwardPass();
                conv7.BackwardAdditionSkipConnection(tran7);
                conv6.BackwardPass();
                conv5.BackwardPass();
                conv4.BackwardPass();
                conv4.BackwardAdditionSkipConnection(tran4);
                conv3.BackwardPass();
                conv2.BackwardPass();
                conv1.BackwardPass();
            }
